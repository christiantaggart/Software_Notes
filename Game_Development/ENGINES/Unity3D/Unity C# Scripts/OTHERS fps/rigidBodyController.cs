// It's heavily based off the FPSController that comes with unity, but simplified down and modified to suit rigidbodies (rigidbody handles lots of things by itself) with other features like wall jumping added.

// REQUIRES an input controller and mouse look. The default FPSInputController.cs and MouseLook.cs ones work fine. Just change GetComponent(); into GetComponent();

// Things to note about momentum:

// -When you jump on a very fast moving platform (even rotating) you'll be thrown off.

// -When you let go of jump you keep moving. Main reason I started on this script.
// -Momentum is mostly overwritten by controls. This was key to having good fast snappy control which my game requires. Easily fine tuned or turned off if you wanted more realistic turning movements.

// It's not designed for realism. It's designed to give good control and play well for my game.
...

{
   ...
   [NonSerialized] private bool _contact;

   [NonSerialized] private float _lastContact;

   [NonSerialized] private bool _grounded;



   [NonSerialized] private Vector3 _contactNormal = Vector3.up;
...

   {
       ...
       var maxVelocityChange = (!_contact || _contactNormal.y
...

   {
       ...
       if (InputJump && Jumping.LastButtonDownTime
           Jumping.LastButtonDownTime = Time.time;
       ...
       if (_contact || !_contact && (Time.time - _lastContact
       {

           if (!Jumping.Jumping && Jumping.Enabled && CanControl && (Time.time - Jumping.LastButtonDownTime
           {
